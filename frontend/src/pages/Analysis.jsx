...
function deriveDirections(features) {
  ...
  const RULES = [
    ...
    {
      keywords: ["llm", "foundation model", "training"],
      label: "Foundation Models",
    },
    {
      keywords: ["tpu", "gpu", "compute", "scale"],
      label: "AI Infrastructure",
    },
    {
      keywords: ["multi-cloud", "cross-cloud", "hybrid"],
      label: "Cloud Consolidation",
    },
    {
      keywords: ["api", "sdk", "developer", "toolkit"],
    ...
    {
      keywords: ["cost", "pricing", "free tier"],
...
function getTopFeatures(features, limit = 4) {
  ...
  return [...features]
    .sort((a, b) => {
      ...
      const diff =
        (impOrder[b.importance] || 1) - (impOrder[a.importance] || 1);
      return diff !== 0
        ? diff
...
function buildComparativeSummary(items) {
  ...
  const parts = items.map((c) => {
    ...
    const topCat = Object.entries(getCategoryDist(c.features)).sort(
      (a, b) => b[1] - a[1]
    )[0];
    const focus =
      dirs.length > 0
        ? dirs[0].toLowerCase()
        : topCat
        ? topCat[0].toLowerCase()
        : "general technology";
    return { name: c.label, focus, signals: c.features?.length || 0 };
  });
  let summary =
    parts.map((s) => `${s.name} focuses on ${s.focus}`).join(", while ") + ". ";
...
function deriveKeyDifferences(items) {
  ...
  items.forEach((c) => {
    ...
    if (top) {
      ...
      diffs.push(
        `${c.label}: "${top.feature_title || top.feature_text}"${
          impact ? ` — ${impact}` : ""
        }`
      );
    }
  ...
  const cats = items.map((c) => {
    ...
    return {
      name: c.label,
      topCat: topCat?.[0] || "General",
      count: topCat?.[1] || 0,
    };
  });
  if (cats[0]?.topCat !== cats[1]?.topCat) {
    diffs.push(
      `${cats[0].name} leads in ${cats[0].topCat} (${cats[0].count} signals), while ${cats[1].name} focuses on ${cats[1].topCat} (${cats[1].count} signals).`
    );
  }
  const byConf = [...items].sort(
    (a, b) => getAvgConf(b.features) - getAvgConf(a.features)
  );
  ...
  if (topConf > 0 && topConf - lowConf > 0.1) {
    diffs.push(
      `${byConf[0].name} shows higher confidence (${(topConf * 100).toFixed(
        0
      )}%) vs ${byConf[byConf.length - 1].name} (${(lowConf * 100).toFixed(
        0
      )}%).`
    );
  }
...
function getCategoryOverlap(items) {
  ...
  const catSets = items.map(
    (c) => new Set(Object.keys(getCategoryDist(c.features)))
  );
  const allCats = new Set(catSets.flatMap((s) => [...s]));
  const shared = [...allCats].filter((cat) => catSets.every((s) => s.has(cat)));
  const unique = {};
  items.forEach((c, i) => {
    unique[c.label] = [...catSets[i]].filter(
      (cat) => !catSets.some((s, j) => j !== i && s.has(cat))
    );
  });
...
export default function Analysis() {
  ...
  async function refreshAnalysis() {
    ...
    // COMPANY MODE
    if (mode === "companies" && selectedCompanies.length >= 2) {
      setLoading(true);
    ...
    // TIMELINE MODE
    if (mode === "timeline" && timelineCompany) {
      ...
      try {
        ...
        if (selectedReports.length >= 2) {
          const updated = selectedReports.map((r) => {
            return arr.find((x) => x.id === r.id) || r;
          });
  ...
  useEffect(() => {
    if (mode !== "timeline" || selectedReports.length < 2) {
      setComparisonData([]);
      return;
    }
  ...
  function addCompany(name) {
    if (
      selectedCompanies.length >= MAX_ITEMS ||
  ...
  const comparativeSummary = useMemo(
    () => buildComparativeSummary(comparisonData),
    [comparisonData]
  );
  const keyDifferences = useMemo(
    () => deriveKeyDifferences(comparisonData),
    [comparisonData]
  );
  const categoryOverlap = useMemo(
    () => getCategoryOverlap(comparisonData),
    [comparisonData]
  );
  ...
  return (
    <div className="analysis-page fade-in">
      <SectionHeader
        ...
        title="Analysis"
        subtitle="Compare competitive intelligence across companies or track a single company over time"
        loading={loading}
        ...
        <div className="card analysis-selector">
          ...
          {loadingList ? (
            ...
            <p className="selector-empty">
              No companies tracked yet. Run the intelligence pipeline first.
            </p>
          ) : (
          ...
          {selectedCompanies.length === 1 && (
            <p className="selector-hint">
              Select at least 2 companies to start analysis.
            </p>
          )}
        </div>
      )}
      ...
      {mode === "timeline" && (
        <div className="card analysis-selector">
          <div className="selector-header">
            ...
            <span className="selector-count">
              {selectedReports.length} / {MAX_ITEMS} reports
            </span>
          </div>
          ...
          {loadingList ? (
            ...
            <div className="timeline-company-select">
              ...
              {available.filter((c) => reportCounts[c.name] >= 2).length ===
              0 ? (
                <p className="selector-hint" style={{ marginTop: 0 }}>
                  No companies have a timeline of reports yet. Run the pipeline
                  multiple times for the same company to build a timeline.
                </p>
              ) : (
                <div className="available-companies">
                  {available
                    .filter((c) => reportCounts[c.name] >= 2)
                    .map((c) => (
                      <button
                        key={c.id}
                        className={`available-chip ${
                          timelineCompany === c.name ? "selected" : ""
                        }`}
                        onClick={() => {
                          setTimelineCompany(c.name);
                          setSelectedReports([]);
                        }}
                      >
                        {c.name}
          ...
          {timelineCompany && (
            <div className="timeline-reports-select">
              ...
              {loadingReports ? (
                ...
                <p className="selector-empty">
                  No reports found for {timelineCompany}.
                </p>
              ) : allReports.length < 2 ? (
                <p className="selector-hint">
                  Only 1 report available. Run the pipeline again on a different
                  day to compare.
                </p>
              ) : (
                <div className="timeline-report-chips">
                  {allReports.map((r) => {
                    ...
                    const idx = selectedReports.findIndex(
                      (sr) => sr.id === r.id
                    );
                    return (
                      <button
                        ...
                        disabled={
                          !isSelected && selectedReports.length >= MAX_ITEMS
                        }
                      ...
                      >
                        ...
                        <span className="tc-meta">
                          {r.total_features || 0} signals ·{" "}
                          {r.total_sources || 0} sources
                        </span>
                      </button>
                    );
                  })}
      ...
      {hasResults && (
        <div className="analysis-results stagger">
                ...
                return (
                  <div key={c.label} className="direction-column">
                    ...
                    <div className="direction-tags">
                      {dirs.length > 0 ? (
                        dirs.map((d, j) => (
                          <span key={j} className="direction-tag">
                            {d}
                          </span>
                        ))
                      ) : (
                        <span className="direction-tag empty">
                          No clear direction
                        </span>
                      )}
                    </div>
          ...
          {(categoryOverlap.shared.length > 0 ||
            Object.values(categoryOverlap.unique).some(
              (u) => u.length > 0
            )) && (
            <div className="card comp-overlap-card fade-in-up">
              <h3 className="comp-section-title">
                <HiOutlineEye /> Category Coverage
              </h3>
              <div className="overlap-layout">
                {categoryOverlap.shared.length > 0 && (
                  <div className="overlap-section">
                    <span className="overlap-label shared">
                      {mode === "timeline"
                        ? "Consistent Focus"
                        : "Shared Focus Areas"}
                    </span>
                    <div className="overlap-tags">
                      {categoryOverlap.shared.map((cat) => (
                        <span key={cat} className="overlap-tag shared">
                          {cat}
                        </span>
                      ))}
                    </div>
                    ...
                    <div key={c.label} className="overlap-section">
                      ...
                      <div className="overlap-tags">
                        {uniqueCats.map((cat) => (
                          <span
                            key={cat}
                            className="overlap-tag unique"
                            style={{
                              borderColor: COLOR_HEX[i] + "40",
                              color: COLOR_HEX[i],
                            }}
                          >
                            {cat}
          ...
          <div className="card comp-signals-card fade-in-up">
            <h3 className="comp-section-title">
              <HiOutlineLightningBolt /> Top Signals Compared
            </h3>
                ...
                return (
                  <div key={c.label} className="signals-column">
                    <div className="signals-col-header">
                      <span
                        className="signals-col-dot"
                        style={{ background: COLORS[i] }}
                      />
                      <span>{c.label}</span>
                      <span className="signals-col-count">
                        {c.features.length} signals
                      </span>
                    </div>
                        ...
                        <div key={j} className="signal-compare-item">
                          <div className="sci-header">
                            <span className="sci-title">
                              {f.feature_title || f.feature_text}
                            </span>
                            {f.importance && (
                              <span
                                className={`sci-importance ${f.importance}`}
                              >
                                {f.importance}
                              </span>
                            )}
                          ...
                          {(f.description || f.feature_text) && (
                            <p className="sci-desc">
                              {(f.description || f.feature_text || "").slice(
                                0,
                                120
                              )}
          ...
          <div className="card comp-strength-card fade-in-up">
            ...
            <div className="strength-grid">
              {comparisonData.map((c, i) => {
                ...
                const highCount = c.features.filter(
                  (f) => f.importance === "high"
                ).length;
                return (
                  <div key={c.label} className="strength-item">
                    ...
                    <div className="strength-info">
                      ...
                      <div className="strength-row">
                        ...
                        <span className="strength-conf-value">
                          {(conf * 100).toFixed(0)}%
                        </span>
                      </div>
                      <div className="strength-row">
                        <span className="strength-conf-label">
                          High Priority
                        </span>
          ...
          <div className="card comp-charts-card fade-in-up">
            <h3 className="comp-section-title">
              <HiOutlineChartBar /> Visual Analytics
            </h3>
